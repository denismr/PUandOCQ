# See ExperimentBFT.py for more information.
...
scorers_by_name = {
  ...
  "7nn": OCScorers.NthNearest(3),
  "nb": OCScorers.NPNB(30),
  "nb10": OCScorers.NPNB(10),
}
...
def RunExperiment(exp_name, scorer_name, niterations=5, max_sample_size=2000, max_labeled_size=500, nfolds=5):
  ...
  for neg_label in negative_labels:
    ...
    for alpha in tqdm(list(np.linspace(0, 1, 11)), desc="alpha"):
      ...
      row = [str(neg_label), 100 * alpha, np.mean(ms_per_example), np.std(ms_per_example)] + list(abs_errors) + list(std_abs_errors) + list(errors) + list(std_errors)
  ...
  header_csv = 'negative,alpha,time,time_std,%s,%s,%s,%s' % (h1, h2, h3, h4)
  mask_csv = ','.join(['%s'] + (['%.2f'] * (len(for_table[0]) - 1)))

  eprint()
  with open(output_filename % ('_BFTRaw_EachNeg', 'bft_raw_%s' % scorer_name), mode="w") as out:
